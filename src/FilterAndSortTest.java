    ...
    public static void main(String[] args) {
        ...
        if (testCase == 1) { // students
            ...
            if (studentScenario == 1) {
                ...
                try {
                    FilterAndSort.execute(students,predicate).forEach(System.out::println);
                ...
                Predicate<Student>predicate = student -> student.average() >= 9.0 && student.grades.size() * 1.0/student.totalCourses() > 0.90;
                try {
                    FilterAndSort.execute(students, predicate).forEach(System.out::println);
                } catch (EmptyResultException e) {
                    System.out.printf("%s ",e.getMessage());
            ...
            Predicate<Integer>predicate = integer -> integer % 15 == 0 && integer % 2 == 0;
            try {
                FilterAndSort.execute(integers, predicate).stream().forEach(System.out::println);
            } catch (EmptyResultException e) {
                System.out.printf("%s ",e.getMessage());
            }
        }
